...
struct Monitor {
	...
	int gappih;           /* horizontal gap between windows */
    	...
    	int gappoh;           /* horizontal gap around windows */
    	int gappov;           /* vertical gap around windows */
};
...
static void viewprevtag(const Arg *);
static void run_tag_scripts(void);
...
static unsigned int lasttag = 1;  // stores previous tag
static unsigned int prevtag = 1;  /* start at tag 1 */
...
viewprevtag(const Arg *arg)
{
    ...
    // Only swap the tagset, do not touch lasttag/prevtag
    unsigned int tmp = selmon->tagset[selmon->seltags];
    selmon->tagset[selmon->seltags] = prevtag;
    prevtag = tmp;

    // Update per-tag layout data
    int curtag = -1;
...
/* call scripts based on the *currently visible* tagset on selmon */
static void
run_tag_scripts(void)
{
    if (!selmon)
        return;
...
view(const Arg *arg)
{
    ...
    unsigned int curtagset = selmon->tagset[selmon->seltags];
    int i;
